...
const variableAndMethodName = () => {
  const text = trimMultiline(textRoot.value);

  let convert = text.split("\n").map((line) => {
    return line
      .trim()
      .replace(/[_\s-]+/g, " ") // Thay thế dấu gạch dưới, khoảng trắng và dấu gạch ngang bằng khoảng trắng
      .split(' ')
    .map((word, index) => {
      if (index === 0) {
        return word.toLowerCase(); // Từ đầu viết thường
      }
  ...
  <div class="lg:col-span-12">
    ...
    <div class="mt-8 pb-24 space-y-12">
      <div id="#automation-and-conventions" class="section">
        <v-textarea
          label="Vui lòng nhập text..."
          variant="outlined"
          rows="10"
          v-model="textRoot"
          @keydown.tab.prevent="insertTab($event, 'textRoot')"
        ></v-textarea>
        <div class="flex flex-col sm:flex-row gap-2 mt-4">
          ...
          <v-btn flat @click="variableAndMethodName">
            <span class="text-ellipsis normal-case">Variable & Method</span>
          </v-btn>
